...
config.TRAIN.g_alpha = 5.0 # weight for pixel loss
config.TRAIN.g_gamma = 0.0025  # weight for perceptual loss
config.TRAIN.g_beta = 5.0 # weight for frequency loss
config.TRAIN.g_adv = 1  # weight for frequency loss

config.TRAIN.seed = 100
config.TRAIN.epsilon = 0.000001

config.TRAIN.episode_size = 5
config.TRAIN.history_size = 30

config.TRAIN.VGG16_path = os.path.join('trained_model', 'VGG16', 'vgg16_weights.npz')
config.TRAIN.training_data_path = os.path.join(path, 'data', 'MICCAI13_SegChallenge', 'training.pickle')
config.TRAIN.val_data_path = os.path.join(path, 'data', 'MICCAI13_SegChallenge', 'validation.pickle')
config.TRAIN.testing_data_path = os.path.join(path, 'data', 'MICCAI13_SegChallenge', 'testing.pickle')
